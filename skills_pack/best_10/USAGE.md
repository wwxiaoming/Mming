...
> 配套文件：`zips/` 10 个独立 zip + `top10_bundle.zip` 总包

---

## 0. 一页速查

| # | Skill | 何时召唤它 | 必备依赖 | 一句话定位 |
|---|-------|-----------|---------|-----------|
| 1 | longbridge-earnings | "TSLA 这季财报怎么看？" | Longbridge CLI | 业绩前瞻 + 回顾 + 电话会 Q&A |
| 2 | akshare | "比亚迪的实时行情""中国 PMI" | `pip install akshare` | 中文金融数据万能瑞士军刀 |
| 3 | longbridge-research | "NVDA 机构目标价多少？" | Longbridge CLI | 机构数据 + 卖方研究框架 |
| 4 | company-valuation | "AAPL 该值多少？建个 DCF" | `pip install yfinance` | DCF + 相对估值 + SOTP |
| 5 | sector-analyst | "现在哪个板块在轮动？" | Python + requests | 板块轮动 + 周期定位 |
| 6 | macro-rates-monitor | "美债曲线倒挂了吗？" | LSEG MCP | 宏观 + 收益率曲线仪表盘 |
| 7 | market-breadth-analyzer | "美股广度健康吗？" | Python + requests | 0-100 广度综合分 |
| 8 | commodities-quote | "黄金现价多少？" | Octagon MCP | 大宗商品实时报价 |
| 9 | market-top-detector | "是不是要减仓了？" | FMP API + WebSearch | 见顶概率 0-100 分 |
| 10 | institutional-flow-tracker | "巴菲特最近加仓了什么？" | FMP API | 13F 机构资金流 |
...
# C. 只装一个
unzip zips/02_18_akshare_succ985.zip -d ~/.claude/skills/
```
