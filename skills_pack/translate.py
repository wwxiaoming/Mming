#!/usr/bin/env python3
"""Translate the `description` field of each SKILL.md to Chinese, then re-zip.

Strategy: parse YAML frontmatter, locate `description:` block, replace with
Chinese version formatted as a literal block scalar (`|`) for readability.
The `name` field is preserved (it's a code identifier). Body is untouched.
"""
import re
import shutil
import zipfile
...
# (skill_dir_name, chinese_description)
# These replace ONLY the frontmatter `description:` value.
TRANSLATIONS = {
...
DESC_LINE_RE = re.compile(r'(^|\n)description:.*$', re.MULTILINE)
DESC_QUOTED_RE = re.compile(r'(^|\n)description:\s*"(.*)"\s*$', re.DOTALL)

def to_block_scalar(text: str) -> str:
    """Convert plain text to a YAML literal block scalar (description: |)."""
    lines = text.split("\n")
    out = [f"  {ln}" if ln.strip() else "  " for ln in lines]
    return "description: |\n" + "\n".join(out) + "\n"

def replace_desc(content: str, new_desc: str) -> str:
    """Replace the description field (in any YAML form) with new_desc as block scalar."""
    m = FM_RE.match(content)
    if not m:
        raise RuntimeError("No YAML frontmatter found")
    fm, body = m.group(1), m.group(2)
    new_block = to_block_scalar(new_desc)

    # Try block scalar first (description: | or >)
    new_fm, n = DESC_BLOCK_RE.subn(lambda _: "\n" + new_block, fm, count=1)
    if n:
        return "---\n" + new_fm + "\n---\n" + body

    # Try quoted single line (description: "...")
    new_fm, n = DESC_QUOTED_RE.subn(lambda _: "\n" + new_block, fm, count=1)
    if n:
        return "---\n" + new_fm + "\n---\n" + body

    # Try plain single line (description: ...)
    new_fm, n = DESC_LINE_RE.subn(lambda _: "\n" + new_block, fm, count=1)
    if n:
        return "---\n" + new_fm + "\n---\n" + body
...
ok, errors = [], []
for skill_name, cn_desc in TRANSLATIONS.items():
    ...
    try:
        updated = replace_desc(original, cn_desc)
    except Exception as e:
        errors.append((skill_name, f"replace failed: {e}"))
        continue

    if updated == original:
        errors.append((skill_name, "no change applied"))
        continue

    skill_md.write_text(updated, encoding="utf-8")

    # Re-zip
    zip_path = ZIPS / f"{skill_name}.zip"
    if zip_path.exists():
        zip_path.unlink()
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
        for p in skill_path.rglob("*"):
            if p.is_file():
                zf.write(p, arcname=f"{skill_name}/{p.relative_to(skill_path)}")
    ok.append((skill_name, zip_path.stat().st_size))

print(f"=== TRANSLATED ({len(ok)}) ===")
for n, sz in ok:
    print(f"  {n:50s} {sz:>10,} bytes")
print(f"\n=== ERRORS ({len(errors)}) ===")
for n, why in errors:
    print(f"  {n:50s} -> {why}")
print(f"\nDone: {len(ok)}/{len(TRANSLATIONS)}")
