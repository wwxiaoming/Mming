...
moneyflow_blp = Blueprint('cn-moneyflow', __name__)
_cache = CacheManager()
...
def _fetch_akshare_moneyflow(symbol: str, days: int) -> Optional[List[Dict[str, Any]]]:
    """调用 AKShare 拉单只股票的资金流。

    返回的 list 元素是按日期降序的最新 days 条。
    """
    ...
    # AKShare 列名(中英文版本略有差异,做兼容)
    # 期望列:日期/收盘价/涨跌幅/主力净流入-净额/主力净流入-净占比
    ...
    # 取最近 days 行
    df = df.head(days).iloc[::-1]  # 翻成正序(从早到晚)
    ...
    for _, row in df.iterrows():
        item: Dict[str, Any] = {
            ...
            "main_net": _to_wan(row.get("主力净流入-净额")),
            "super_net": _to_wan(row.get("超大单净流入-净额")),
            "big_net": _to_wan(row.get("大单净流入-净额")),
            "mid_net": _to_wan(row.get("中单净流入-净额")),
            "small_net": _to_wan(row.get("小单净流入-净额")),
        }
        # 全单净流入 = 主力 + 中单 + 小单(粗略)
        if item["main_net"] is not None and item["mid_net"] is not None and item["small_net"] is not None:
            item["total_net"] = round(
                (item["main_net"] or 0) + (item["mid_net"] or 0) + (item["small_net"] or 0), 2
            )
        else:
            item["total_net"] = None
        out.append(item)
