"""
daily_stock_pick.py — 9 策略并行选股（x1.0 融合版）
策略清单:
...
x1.0 新增（相对 v1.7.0）:
  - 环境闸门（x1.environment_gate）：S/A/B/C/D 评级，D 级直接出空仓报告
...
def strategy_8_potential5() -> list[dict]:
    ...
    for stock in passed_stocks:
        ...
        # 1) 位置(温和,未暴涨)
        if -3.0 <= chg <= 3.0:
            position_score = 1.0 - abs(chg) / 3.0
        elif 3.0 < chg <= 6.0:
            position_score = 0.3
        else:
            position_score = -0.5
        ...
        # 3) 资金
        if 1.5 <= turnover <= 8.0:
            fund_score = 1.0 - abs(turnover - 4) / 4
        ...
        us_local = us_score * 2.0 if is_tech else us_score * 0.5
    ...
    # x1.0 Step 4: 4 选 1 结论映射
    for r in top5:
        sub = {
            "position": r["breakdown"].get("position(0.20)", 0),
            "fund":     r["breakdown"].get("fund(0.30)", 0),
            "theme":    r["breakdown"].get("theme(0.20)", 0),
        }
        conc = map_conclusion(r["score"], sub)
        r["conclusion"] = conc["conclusion"]
...
def main():
    ...
    if args.mode == "weekend":
        ...
        picks = strategy_weekend_pick() if not env["skip_stock_pick"] else []
        # 给周末 TOP 5 也接 4 选 1 结论
        for r in picks:
            sub = {"position": 0.5, "fund": 0.5, "theme": 0.5}
            conc = map_conclusion(r["score"], sub)
            r["conclusion"] = conc["conclusion"]
            r["conclusion_emoji"] = conc["emoji"]
            r["condition_trigger"] = conc["condition_trigger"]
            r["edge_flag"] = conc["edge_flag"]
        all_results["weekend_pick"] = picks
