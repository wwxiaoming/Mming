...
def write_summary(today: str, us: dict, picks: dict, t159: dict) -> str:
    ...
    # x1.0: 策略 8 明日潜力股 TOP 5 + 4 选 1 结论
    potential5 = picks.get("8_potential5", [])[:5]
    if potential5:
        lines.append("8️⃣ 🌱 明日潜力股 TOP 5(x1.0 4 选 1)")
        for i, r in enumerate(potential5, 1):
            emoji = r.get("conclusion_emoji", "⚪")
            conc = r.get("conclusion", "")
            lines.append(f"  {i}. {r['code']} {r['name']} 分={r['score']} {emoji} {conc}")
        lines.append("")

    # x1.0: 环境闸门
    x1m = picks.get("x1_meta", {})
    if x1m:
        lines.append("🚦 环境闸门(x1.0):")
        lines.append(f"  评级 {x1m.get('environment_grade', '?')} / {x1m.get('position_desc', '?')}")
        lines.append("")

    lines.append("📁 详细报告: /workspace/daily_picks/{}/".format(today))
    lines.append("📁 x1.0 主报告: /workspace/output/{}_早盘潜力股_x1.0.md".format(today))
    return "\n".join(lines)

# ── 2. 写 state.json(给前端 / 监控消费)─
def write_state_json(today: str, us: dict, picks: dict, t159: dict):
    # x1.0: 在 potential5 每条记录中加 conclusion 字段
    p5 = picks.get("8_potential5", [])[:5]
    potential5_with_conclusion = []
    for r in p5:
        potential5_with_conclusion.append({
            **r,
            "conclusion":      r.get("conclusion", ""),
            "conclusion_emoji":r.get("conclusion_emoji", "⚪"),
            "condition_trigger":r.get("condition_trigger", ""),
            "edge_flag":       r.get("edge_flag", False),
        })
    state = {
        "date": today,
        "generated_at": now_iso(),
        "version": "x1.0",   # x1.0 标记
        "x1_meta": picks.get("x1_meta", {}),  # x1.0 闸门+结论摘要
        ...
        "top_picks": {
            ...
            "potential5": potential5_with_conclusion,  # x1.0: 含 conclusion
