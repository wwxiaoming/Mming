#!/usr/bin/env bash
# run_weekend.sh — x1.0 融合版周末模式（周日晚 22:00 跑，选周一开盘潜力股）
# 流程:
#   1. 美股周五收盘（us_market_fetcher）
#   2. 周末选股（daily_stock_pick.py --mode=weekend，含闸门 + 排除 + 4 选 1 映射）
#   3. 拼周末报告（post_report_weekend.py，x1.0 四合一结构）
#   4. 6 通道写出（auto_publish.py）
# 任务 ID: Z54UKZQHEQYOIE（x1.0 周末展望）
# x1.0 新增: 同工作日（环境闸门 + 10 条排除 + 4 选 1 结论）
set -e
cd /workspace
