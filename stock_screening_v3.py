...
def get_intraday_tencent(code):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if 'data' in minute_data and minute_data['data']:
                rows = []
                for item in minute_data['data']:
                    parts = item.split()
                    if len(parts) >= 4:
                        rows.append({
                            'time': parts[0],       # "0930", "0931", ..., "1457", "1458"
                            'price': float(parts[1]),
...
def check_tail_sneak(intraday_df):
    ...
    try:
        ...
        max_surge = 0
        window = min(5, len(prices) - 1)  # 5分钟窗口
...
def check_volume_sustained(intraday_df):
    ...
    try:
        ...
        # 上午数据: 09:30-11:30, 下午数据: 13:00-15:00
        ...
        # 下午/上午比率，正常应>0.6（下午时间短一些）
        ratio = afternoon_amount / morning_amount
        # 如果下午成交额不到上午的50%则视为萎缩
        passed = ratio >= 0.5
